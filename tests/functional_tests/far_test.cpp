...
TEST_CASE("error regexp", "[far]")
{
    auto f = make_scanner<scan_mode::regex>(R"(((()))", "");
    REQUIRE(f == false);
}

TEST_CASE("a sample lazy loop", "[far]")
{
    SECTION("char const & [N]")
    {
