...
template <typename I, typename C>
concept forward_iterative = iterative<I, C> && std::forward_iterator<I>;
...
template <typename T, typename I> struct operator_string_view
{};
    ...
    , operator_string_view<iterator_pair<I>, I>
{
    ...
    decltype(auto) constexpr begin() noexcept
    {
        return this->first;
    }
...
template <mode M, char_type C> struct rule;

template <char_type C> struct rule<mode::basic, C> : basic_or_icase_rule<C>
{
...
template <char_type C> struct rule<mode::regex, C>
{
...
template <mode M, char_type C, bidirectional_iterative<C> I> class generator;
